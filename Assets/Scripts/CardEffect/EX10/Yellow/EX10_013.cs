    ...
    public class EX10_013 : CEntity_Effect
    {
        public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
        {
            ...
            if (timing == EffectTiming.OnEnterFieldAnyone)
            {
                ...
                activateClass.SetUpICardEffect("Move this Digimon to battle area", CanUseCondition, card);
                ...
                {
                    return CardEffectCommons.IsExistOnBreedingAreaTrigger(card,activateClass)
                        && CardEffectCommons.CanTriggerWhenDigivolving(hashtable, card);
                }

                bool CanActivateCondition(Hashtable hashtable)
                {
                    return CardEffectCommons.IsExistOnBreedingAreaActivate(card, activateClass)
            ...
            if (timing == EffectTiming.OnEndTurn)
            {
                ...
                bool CanActivateCondition(Hashtable hashtable)
                {
                    return CardEffectCommons.IsExistOnBattleAreaActivate(card, activateClass)
                        && CardEffectCommons.MatchConditionOwnersCardCountInTrash(card, IsLucemon) >= 5;
                }
